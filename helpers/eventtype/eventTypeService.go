...
func (s *EventTypeService) GetAll(query models.SearchEventTypeDto) (interface{}, error) {
	...
	selectClause := "DISTINCT et.eventtype_uuid, et.name, et.slug, toString(et.groups) as groups"
	...
	for rows.Next() {
		...
		if err := rows.Scan(&eventType.ID, &eventType.Name, &eventType.Slug, &groupsStr); err != nil {
			return nil, middleware.NewInternalServerError("Something went wrong", err.Error())
		}
